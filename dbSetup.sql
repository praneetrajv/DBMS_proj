...
-- Procedure: Get User's Friends' Posts
DELIMITER //
...
DELIMITER //
CREATE TRIGGER increment_like_count AFTER INSERT ON LikeTable
FOR EACH ROW
BEGIN
    UPDATE Post SET LikeCount = LikeCount + 1 WHERE PostID = NEW.PostID;
END//

CREATE TRIGGER decrement_like_count AFTER DELETE ON LikeTable
FOR EACH ROW
BEGIN
    UPDATE Post SET LikeCount = LikeCount - 1 WHERE PostID = OLD.PostID;
END//

-- TRIGGERS FOR COMMENT COUNT
CREATE TRIGGER increment_comment_count AFTER INSERT ON Comment
FOR EACH ROW
BEGIN
    UPDATE Post SET CommentCount = CommentCount + 1 WHERE PostID = NEW.PostID;
END//

CREATE TRIGGER decrement_comment_count AFTER DELETE ON Comment
FOR EACH ROW
BEGIN
    UPDATE Post SET CommentCount = CommentCount - 1 WHERE PostID = OLD.PostID;
END//

-- TRIGGERS FOR GROUP MEMBER COUNT
CREATE TRIGGER increment_member_count AFTER INSERT ON GroupMembership
FOR EACH ROW
BEGIN
    UPDATE GroupTable SET MemberCount = MemberCount + 1 WHERE GroupID = NEW.GroupID;
END//

CREATE TRIGGER decrement_member_count AFTER DELETE ON GroupMembership
FOR EACH ROW
BEGIN
    UPDATE GroupTable SET MemberCount = MemberCount - 1 WHERE GroupID = OLD.GroupID;
END//
